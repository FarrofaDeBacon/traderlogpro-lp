...
<section id="how-it-works" class="py-24 px-6 bg-slate-900 border-y border-white/5 z-10 relative">
    <div class="max-w-6xl mx-auto space-y-16">
        <div class="text-center space-y-4 max-w-2xl mx-auto">
            <h2 class="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-400">Fluxo Real</h2>
            <h3 class="font-outfit text-4xl md:text-5xl font-black tracking-tight text-white uppercase">COMO FUNCIONA</h3>
            <p class="text-slate-400 font-medium">Processo simples e direto para registrar e evoluir no trading de forma consistente.</p>
        </div>

        <div class="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {#each steps as step}
                <div class="p-8 bg-slate-950/60 border border-white/5 rounded-[2.5rem] relative space-y-4 hover:bg-slate-800/40 hover:border-emerald-500/40 hover:shadow-2xl hover:shadow-emerald-500/5 transition-all duration-300">
                    <span class="text-4xl font-outfit font-black text-emerald-500/20 absolute top-6 right-8 select-none">{step.num}</span>
                    <span class="text-[8px] bg-slate-900 border border-white/10 text-slate-400 px-3 py-1 rounded-full font-black uppercase tracking-wider">{step.screen}</span>
